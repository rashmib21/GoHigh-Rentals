# ─────────────────────────────────────────────
# booking.py  –  Booking & Document routes
# ─────────────────────────────────────────────
# Handles: document submission, booking creation,
# bill display, booking cancellation, vehicle rating
# ─────────────────────────────────────────────
...
# Where uploaded document files are saved
...
def get_cancellation_deduction(travel_date_str, total_amount):
    """Calculate cancellation fee based on how soon the trip is."""
...
def get_security_deposit(vehicle_type):
    """Get security deposit amount for a vehicle type."""
    return SECURITY_DEPOSIT.get(vehicle_type, 3000)


# ──────────────────────────────────────
# Route: Submit Documents (Aadhaar + DL)
# ──────────────────────────────────────
...
def submit_documents():
    ...
    if request.method == "POST":
        ...
        # Validate DL format: 2 state letters + 2 RTO digits + 4 year digits + 7 digits
        ...
        # ── Cross-check: reject if another user already has this Aadhar or DL ──
        try:
            cursor.execute("""
                SELECT user_id, aadhar_number, dl_number FROM user_documents
                WHERE user_id != %s AND (aadhar_number = %s OR dl_number = %s)
            """, (session['user_id'], aadhar_number, dl_number))
            duplicate = cursor.fetchone()
            if duplicate:
                if duplicate['aadhar_number'] == aadhar_number:
                    flash("This Aadhaar number is already registered with another account.", "error")
                else:
                    flash("This Driving Licence number is already registered with another account.", "error")
                cursor.close()
                conn.close()
                return redirect(url_for("booking.submit_documents"))
        except Exception:
        ...
        # Create a safe filename from user's name
        ...
        if aadhar_file and aadhar_file.filename and allowed_file(aadhar_file.filename):
            ...
            aadhar_filename = f"{safe_name}_aadhar.{ext}"
            file_path = os.path.join(UPLOAD_FOLDER, aadhar_filename)

            # Check file size (max 2MB)
            aadhar_file.seek(0, 2)  # Go to end to check size
            file_size = aadhar_file.tell()
            aadhar_file.seek(0)     # Go back to start
        ...
        if dl_file and dl_file.filename and allowed_file(dl_file.filename):
            ...
            dl_filename = f"{safe_name}_driving_licence.{ext}"
            file_path = os.path.join(UPLOAD_FOLDER, dl_filename)

            # Check file size (max 2MB)
            dl_file.seek(0, 2)
            file_size = dl_file.tell()
            dl_file.seek(0)
            if file_size > 2 * 1024 * 1024:
                ...
                conn.close()
                return redirect(url_for("booking.submit_documents"))
            dl_file.save(file_path)

        # Check if user already has documents in DB
        ...
        cursor.close()
        conn.close()
        flash("Documents submitted successfully! Verification is in progress.", "success")
        return redirect(url_for("booking.doc_pending"))

    # GET request: show form with existing docs if any
...
# ──────────────────────────────────────
# Route: Document Verification Pending
# ──────────────────────────────────────
@booking_bp.route("/doc_pending")
def doc_pending():
    if "user_id" not in session:
        return redirect("/login")
    ...
    conn = get_db_connection()
    if conn is None:
        return render_template("doc_pending.html")
    cursor = conn.cursor(dictionary=True)

    # Check if documents are already verified
    cursor.execute("SELECT verified FROM user_documents WHERE user_id=%s", (session['user_id'],))
    doc = cursor.fetchone()
    cursor.close()
    conn.close()

    # If already verified, send them to booking page
    if doc and doc['verified'] == 1:
        flash("Your documents are verified! You can now book.", "success")
        return redirect(url_for("booking.create_booking_page"))

    # Check if rejected (verified=2)
    is_rejected = doc and doc['verified'] == 2

    return render_template("doc_pending.html", is_rejected=is_rejected)


# ──────────────────────────────────────
# Route: New Booking Page
# ──────────────────────────────────────
...
def create_booking_page():
    ...
    # Check if user has submitted and verified documents
    try:
        cursor.execute("SELECT doc_id, verified FROM user_documents WHERE user_id=%s", (session['user_id'],))
        doc = cursor.fetchone()
    except Exception:
        doc = {'verified': 1}  # Table not yet created — allow booking
    ...
    if doc.get('verified', 0) != 1:
        cursor.close()
        conn.close()
        return redirect(url_for("booking.doc_pending"))

    # Get which destination was selected (if any)
...
def create_booking():
    ...
    # Validate travel date (must be tomorrow to 30 days from now)
    ...
    cursor.execute(
        "SELECT availability_status, IFNULL(vehicle_count,1) AS vehicle_count FROM vehicle WHERE vehicle_id=%s",
        (vehicle_id,)
    )
    ...
    # Use vehicle price if set, otherwise use destination price
    ...
    gst_amount     = round(base_amount * 0.18, 2)       # 18% GST
...
def show_bill(booking_id):
    ...
    if bill:
        ...
        # Convert time fields from timedelta to string
...
def cancel_booking(booking_id):
    ...
    # Save cancellation details in pricing
    try:
        cursor.execute("""
...
def rate_vehicle(booking_id):
    ...
    try:
        ...
        # Update vehicle table with new rating
        cursor.execute("""
