...
# ─────────────────────────────────────────────
# Handles: admin login/logout, dashboard,
# manage bookings, vehicles, documents,
# inquiries, users, and reviews
# ─────────────────────────────────────────────
...
def allowed_photo(filename):
    """Check if the file extension is allowed for photos."""
...
def admin_login():
    ...
    # If a regular user session exists, clear it
    ...
    if request.method == 'POST':
        ...
        if conn is not None:
            ...
            try:
                cursor.execute(
                    "SELECT * FROM admin_accounts WHERE username=%s OR email=%s",
                    (username, username)
                )
        ...
        # Fallback: try hardcoded admin credentials
        if not admin_found:
...
def admin_dashboard():
    ...
    # Count user-cancelled bookings that admin hasn't seen yet
    cursor.execute("""
...
def admin_dismiss_cancel(booking_id):
    """Mark a cancellation notification as seen."""
    if not admin_required():
        return redirect(url_for("admin.admin_login"))
...
def admin_accept_booking(booking_id):
    """Accept a pending booking → set status to Confirmed."""
    if not admin_required():
        return redirect(url_for("admin.admin_login"))
    ...
    # Don't modify user-cancelled bookings
    ...
    if booking and booking['booking_status'] != 'Pending':
        flash("Only Pending bookings can be accepted.", "error")
        cursor.close()
        conn.close()
        return redirect(url_for("admin.admin_bookings"))
...
def admin_cancel_booking(booking_id):
    """Cancel a pending booking from admin side."""
    if not admin_required():
        return redirect(url_for("admin.admin_login"))
    ...
    if request.method == "POST":
        ...
        if photo_file and photo_file.filename and allowed_photo(photo_file.filename):
            ...
            photo_filename = secure_filename(
                f"vehicle_{request.form.get('vehicle_number', 'unknown')}.{ext}"
            )
...
def admin_edit_vehicle(vehicle_id):
    ...
    if request.method == "POST":
        ...
        # Update with or without new photo
        if photo_url:
            cursor.execute("""
                UPDATE vehicle
    ...
    # GET request: load vehicle data for the edit form
    cursor.execute("""
        SELECT vehicle_id, vehicle_name, vehicle_number, vehicle_type,
...
@admin_bp.route("/admin/document/reject/<int:doc_id>", methods=["POST"])
def admin_reject_document(doc_id):
    """Mark a document as rejected (verified=2)."""
    if not admin_required():
        return redirect(url_for("admin.admin_login"))
    ...
    cursor.execute("UPDATE user_documents SET verified=2 WHERE doc_id=%s", (doc_id,))
    conn.commit()
    cursor.close()
    conn.close()

    flash("Document rejected. User will need to resubmit.", "error")
    return redirect(url_for("admin.admin_documents"))
...
def admin_delete_user(user_id):
    ...
    # Delete related pricing, bookings, then the user
    cursor.execute("""
